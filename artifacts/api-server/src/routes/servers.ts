...
router.post("/servers", async (req, res) => {
  ...
  await auditFromReq(req, {
    ...
    metadata: { version: data.version, serverType: data.serverType },
    level: "info",
  });
...
router.post("/servers/:id/power", async (req, res) => {
  ...
  switch (action) {
    case "start":   serverManager.startServer(server.id, server.memory); break;
    case "stop":    serverManager.stopServer(server.id); break;
    case "restart": serverManager.restartServer(server.id, server.memory); break;
    case "kill":    serverManager.killServer(server.id); break;
  }
